...
setwd("C:/Users/e5103562/OneDrive - Victoria University/EWAS meta analysis of VO2max/")
...
#View results including all 5 studies
...
nrow(meta_res_robust %>% filter(Direction=="Hypo"))/nDMPs #% hypo DMPs = 46% hypo
...
DMPs <- read_delim("C:/Users/e5103562/OneDrive - Victoria University/Tables & Figures/DNAm.txt")$CpG
subL <- L[intersect(names(L),DMPs)] #3166/3207 DMPs tested

models <- t(sapply(subL,
                 meta_RE))
CpG <- intersect(names(L),DMPs)
...
#Pathway enrichment within the age-related DMPs
library(tidyverse)
DNAm <- read_tsv('/Users/wrq796/OneDrive - Victoria University/Tables & Figures/DNAm.txt')

#Replace NA with no name
DNAm$`Annotated gene(s)`[is.na(DNAm$`Annotated gene(s)`)]=""

library(missMethyl)
#OUR own annotation for each CpG
RSanno <- DataFrame(chr = DNAm$Chromosome,
                    pos = DNAm$`Position (hg38)`,
                    Name = DNAm$CpG,
                    UCSC_RefGene_Name = DNAm$`Annotated gene(s)`,
                    UCSC_RefGene_Group = DNAm$`Annotated gene(s)`,
                    row.names = DNAm$CpG)
